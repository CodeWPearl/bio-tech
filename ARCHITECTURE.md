...
1. `src/data/` — download ClinVar + cBioPortal, align, split by gene.
2. `src/features/` — per-modality feature extraction.
3. `src/models/encoders/` — the four modality encoders.
4. `src/models/fusion/` — the five fusion strategies.
5. `src/training/` — Lightning training loop + focal loss.
6. `src/evaluation/`, `src/explainability/`, `src/uncertainty/` — analysis layers.
