...
class APIClient:
    ...
    def get_genes(_self) -> list:
        """Fetch list of genes from the API (cached 5 minutes)."""
        result = _self._request("GET", "/genes", timeout=10)
    ...
    def get_stats(_self) -> dict:
        """Fetch dataset statistics (cached 5 minutes)."""
        result = _self._request("GET", "/stats", timeout=10)
        return result if isinstance(result, dict) else {}

    @st.cache_data(ttl=300, show_spinner=False)
    def get_model_info(_self) -> dict:
        """Fetch model architecture and training info (cached 5 minutes)."""
        result = _self._request("GET", "/model/info", timeout=10)
        return result if isinstance(result, dict) else {}
